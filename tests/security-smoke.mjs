...
try {
  ...
  const staffLogin = await json("/api/admin/login", "staff.nenosensei.com", { method: "POST", body: { username: "owner", password } }); assert(staffLogin.response.ok && staffLogin.data.mfaRequired && !staffLogin.response.headers.get("set-cookie")?.startsWith("neno_admin="), "password created a full staff session without MFA");
