...
try {
  ...
  try { Invoke-RestMethod "$base/api/admin/work-orders/$orderId/contact-log" -Method Post -WebSession $session -Headers $headers -ContentType "application/json" -Body (@{ outcome = "invalid"; notes = "" } | ConvertTo-Json) | Out-Null } catch { if ($_.Exception.Response.StatusCode.value__ -ne 400) { throw }; $invalidContactBlocked = $true }
